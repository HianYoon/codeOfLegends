...
<section id="content" class="page-forum">
						...
						<div class="box-text">
							<div class="box-text-top">
								<div class="wrap-left">
									...
									<p class="date writeDateBefore2"><fmt:formatDate value="${l.WRITTEN_DATE }" pattern="yyyy년 MM월 DD일 HH시 mm분"/></p>
									<p class="date writtenDate2" style="display:none"><fmt:formatDate value="${l.WRITTEN_DATE }" pattern="yyyy-MM-DD'T'HH:mm:ss"/></p>
								...
								<div class="box-text-bottom">
									<dl class="info-article">
										...
										<c:if test="${list.size() != 1 }">
											<div><dt>마지막 댓글</dt><dd id="lastestWriteDate"></dd></div>
										</c:if>
										<c:if test="${list.size() == 1 }">
	...
	<script>
		...
		const writtenDate2 = document.getElementsByClassName('writtenDate2').innerText;
		...
		//document.getElementByClass('writeDateBefore2')[1].innerText = timeForToday(writtenDate);
		document.getElementById('lastestWriteDate').innerText = timeForToday(lastestWrittenDate);
		var comments = document.getElementsByClassName('writeDateBefore2');
		for(let i = 0; i<${list.size()-1}; i++){
			const writtenDate2 = document.getElementsByClassName('writtenDate2')[i+1].innerText;
			document.getElementsByClassName('writeDateBefore2')[i+1].innerHTML = timeForToday(writtenDate2);
		}
		/* 1995-12-17T03:24:00 */
		...
		function chk_file_type(obj) {
			...
			if(check_file_type.indexOf(file_type)==-1) {
				...
				document.getElementById("imageCheck").value = "";
				document.getElementById("imageCheck").select();
