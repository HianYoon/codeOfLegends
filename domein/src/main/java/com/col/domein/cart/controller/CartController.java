...
public class CartController {
	...
	public ModelAndView cart( Cart c,ProductAll p,ModelAndView mv,Member member,HttpSession session
			){
		...
		int memberKey=Integer.valueOf(member.getMemberKey());
		System.out.println("memberKey"+memberKey);
		if(c.getMemberKey()!=memberKey) {
			for(int i=1; i<=1;i++) {
			...
			mv.setViewName("cart/cart");
				...
				int result=service.insertMemberCart(c);
				mv.addObject("memberCart",result);
				mv.setViewName("cart/cart");
			}
